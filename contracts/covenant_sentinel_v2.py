...
# Covenant Sentinel v2 - a Milestone update to contracts/covenant_sentinel.py
# (kept untouched for provenance; this is a new, separate contract, not a
# migration in place). Adds three things v1's own README names as Known
# limitations: (a) a lender-waiver + borrower-cure path out of a one-way
# breach, (b) submit_disclosure_url so a disclosure can be fetched live
# instead of only pasted as text, and (c) a tolerance-band custom validator
# replacing strict_eq's byte-identical extraction, so a metric landing near
# its threshold reports INCONCLUSIVE instead of either forcing a side or
# failing to reach consensus at all.
#
...
# same reason every sibling project in this account's work keeps its
# locally-tested source on this generation. The live deployment target is
# GenLayer Studio Next: contracts/covenant_sentinel_v2_studio_next.py is
# the mechanical port used for that (see its own header), validated live
# only, exactly like the sibling projects' *_studio_next.py files.
...
class CovenantSentinelV2(gl.Contract):
    owner: Address
    borrower: Address
